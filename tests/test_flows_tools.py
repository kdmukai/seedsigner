...
from seedsigner.views.view import ErrorView, MainMenuView, NotYetImplementedView
...
class TestToolsFlows(FlowTest):
    ...
    def test__addressexplorer__custom_derivation__account__flow(self):
        """
            AddressExplorer should support accounts via a custom derivation path for a
            supported single sig "m/{purpose}'/..."
        """
        controller = Controller.get_instance()
        seed = Seed(mnemonic=["abandon "* 11 + "about"])
        controller.storage.set_pending_seed(seed)
        ...
        # Ensure custom derivation is enabled
        self.settings.set_value(SettingsConstants.SETTING__SCRIPT_TYPES, [st[0] for st in SettingsConstants.ALL_SCRIPT_TYPES])

        # test data from bitcoiner.guide/seed; receive addr #14
        script_types = [
            ("84h", "bc1qxjs3lfaar4n7vyzjrkjqml0r0l76nyttygvmsr"),
            ("49h", "3CigFHq9bKfDDWdars6wJhoNwsC5H8XHsK"), 
            ("86h", "bc1pr32qyv8sz43mlnlp7n9jl6jmda9mdjp5gsx49jd4uugzln6aupgsu3d2af"),
        ]

        def verify_address(view: tools_views.ToolsAddressExplorerAddressView):
            assert view.address == cur_target_address

        for script_type, target_address in script_types:
            cur_target_address = target_address
            self.run_sequence([
                FlowStep(MainMenuView, button_data_selection=MainMenuView.TOOLS),
                FlowStep(tools_views.ToolsMenuView, button_data_selection=tools_views.ToolsMenuView.EXPLORER),
                FlowStep(tools_views.ToolsAddressExplorerSelectSourceView, screen_return_value=0),  # ret 1st onboard seed
                FlowStep(seed_views.SeedExportXpubScriptTypeView, button_data_selection=SettingsDefinition.get_settings_entry(SettingsConstants.SETTING__SCRIPT_TYPES).get_selection_option_display_name_by_value(SettingsConstants.CUSTOM_DERIVATION)),
                FlowStep(seed_views.SeedExportXpubCustomDerivationView, screen_return_value=f"m/{script_type}/0h/4h"),  # ret custom derivation path; account 1
                FlowStep(tools_views.ToolsAddressExplorerCustomDerivationRoutingView, is_redirect=True),
                FlowStep(tools_views.ToolsAddressExplorerAddressTypeView, button_data_selection=tools_views.ToolsAddressExplorerAddressTypeView.RECEIVE),
                FlowStep(tools_views.ToolsAddressExplorerAddressListView, screen_return_value=10),  # ret NEXT page of addrs
                FlowStep(tools_views.ToolsAddressExplorerAddressListView, screen_return_value=4),  # ret a specific addr from the list; receive index #14
                FlowStep(tools_views.ToolsAddressExplorerAddressView, before_run=verify_address),  # runs until dismissed; no ret value
                FlowStep(tools_views.ToolsAddressExplorerAddressListView),
            ])


    def test__addressexplorer__custom_derivation__test_network__flow(self):
        """
            AddressExplorer must ask the user to specify which test network to use when
            a custom derivation path with a test network ("m/{purpose}'/1'/...") is used.
        """
        controller = Controller.get_instance()
        seed = Seed(mnemonic=["abandon "* 11 + "about"])
        controller.storage.set_pending_seed(seed)
        ...
        TESTNET = tools_views.ToolsAddressExplorerCustomDerivationSelectTestNetworkView.TESTNET
        REGTEST = tools_views.ToolsAddressExplorerCustomDerivationSelectTestNetworkView.REGTEST

        # test data from iancoleman.io/bip39 and bitcoiner.guide/seed; receive addr #14
        test_data = [
            (TESTNET, "84h", "tb1qeawfs9tuc0vmngztnk0vn6xyx2cx3eznsyl86d"),
            (TESTNET, "49h", "2MujxWKpuKyEeEz8WAB3RAHfF5ijHTCJE1M"), 
            (TESTNET, "86h", "tb1pnan5mun7pyvqxenxza2va2ppw9e3p2pswwt2qn846rkzs7ut4n8sytuwly"),
            (REGTEST, "84h", "bcrt1qeawfs9tuc0vmngztnk0vn6xyx2cx3eznjdx2dy"),
            (REGTEST, "49h", "2MujxWKpuKyEeEz8WAB3RAHfF5ijHTCJE1M"),  # same as testnet

            # bitcoiner.guide doesn't support regtest; iancoleman doesn't support p2tr.
            # So this address was generated by the same code we're testing. But result is
            # the same as TESTNET, minus the "bcrt" prefix and the end checksum.
            (REGTEST, "86h", "bcrt1pnan5mun7pyvqxenxza2va2ppw9e3p2pswwt2qn846rkzs7ut4n8sfjkg27"),  
        ]

        def verify_address(view: tools_views.ToolsAddressExplorerAddressView):
            assert view.address == cur_target_address

        # Run the sequence for both test networks
        for network, script_type, target_address in test_data:
            cur_target_address = target_address
            self.run_sequence([
                FlowStep(MainMenuView, button_data_selection=MainMenuView.TOOLS),
                FlowStep(tools_views.ToolsMenuView, button_data_selection=tools_views.ToolsMenuView.EXPLORER),
                FlowStep(tools_views.ToolsAddressExplorerSelectSourceView, screen_return_value=0),  # ret 1st onboard seed
                FlowStep(seed_views.SeedExportXpubScriptTypeView, button_data_selection=SettingsDefinition.get_settings_entry(SettingsConstants.SETTING__SCRIPT_TYPES).get_selection_option_display_name_by_value(SettingsConstants.CUSTOM_DERIVATION)),
                FlowStep(seed_views.SeedExportXpubCustomDerivationView, screen_return_value=f"m/{script_type}/1h/3h"),  # ret custom derivation path w/test network
                FlowStep(tools_views.ToolsAddressExplorerCustomDerivationRoutingView, is_redirect=True),
                FlowStep(tools_views.ToolsAddressExplorerCustomDerivationSelectTestNetworkView, button_data_selection=network),
                FlowStep(tools_views.ToolsAddressExplorerAddressTypeView, button_data_selection=tools_views.ToolsAddressExplorerAddressTypeView.RECEIVE),
                FlowStep(tools_views.ToolsAddressExplorerAddressListView, screen_return_value=10),  # ret NEXT page of addrs
                FlowStep(tools_views.ToolsAddressExplorerAddressListView, screen_return_value=4),  # ret a specific addr from the list; receive index #14
                FlowStep(tools_views.ToolsAddressExplorerAddressView, before_run=verify_address),  # runs until dismissed; no ret value
    ...
    def test__addressexplorer__custom_derivation___unsupported_purpose__flow(self):
        """
            AddressExplorer will not continue if an unsupported "m/{purpose}'/..." is
            specified. This includes multisig derivation paths that are supported in
            other contexts (multisig Address Explorer requires descriptors instead).
        """
        controller = Controller.get_instance()
        seed = Seed(mnemonic=["abandon "* 11 + "about"])
        controller.storage.set_pending_seed(seed)
        ...
        purposes = [
            ("999h", NotYetImplementedView),  # totally unrecognized
            ("45h",  NotYetImplementedView),  # legacy non-segwit multisig; TODO: change this to ErrorView when m/45' is supported.
            ("48h",  ErrorView),              # multisig not supported
        ]

        for purpose, expected_error in purposes:
            self.run_sequence([
                FlowStep(MainMenuView, button_data_selection=MainMenuView.TOOLS),
                FlowStep(tools_views.ToolsMenuView, button_data_selection=tools_views.ToolsMenuView.EXPLORER),
                FlowStep(tools_views.ToolsAddressExplorerSelectSourceView, screen_return_value=0),  # ret 1st onboard seed
                FlowStep(seed_views.SeedExportXpubScriptTypeView, button_data_selection=SettingsDefinition.get_settings_entry(SettingsConstants.SETTING__SCRIPT_TYPES).get_selection_option_display_name_by_value(SettingsConstants.CUSTOM_DERIVATION)),
                FlowStep(seed_views.SeedExportXpubCustomDerivationView, screen_return_value=f"m/{purpose}/1h/3h"),  # ret custom derivation path w/test network
                FlowStep(tools_views.ToolsAddressExplorerCustomDerivationRoutingView, is_redirect=True),
                FlowStep(expected_error),
            ])
